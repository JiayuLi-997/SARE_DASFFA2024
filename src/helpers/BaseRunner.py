...
class BaseRunner(object):
	...
	def train(self, data_dict, writer):
		model = data_dict['train'].model
		main_metric_results, dev_results = list(), list()
		self._check_time(start=True)
		try:
			for epoch in range(self.epoch):
				data_dict['train'].status='train'
				# Fit
				self._check_time()
				gc.collect()
				torch.cuda.empty_cache()
				loss = self.fit(data_dict['train'], epoch=epoch + 1, writer=writer)
				if np.isnan(loss):
				...
				# Record dev results
				dev_result = self.evaluate(data_dict['dev'], self.topk[:1], self.metrics, writer=writer, epoch=epoch)
				...
				# train results
				train_result = self.evaluate(data_dict['train'],self.topk[:1],self.metrics, writer=writer, epoch=epoch)
				logging_str += ' train=({})'.format(utils.format_metric(train_result))

				# Test
				if self.test_epoch > 0 and epoch % self.test_epoch  == 0:
					test_result = self.evaluate(data_dict['test'], self.topk[:1], self.metrics, writer=writer, epoch=epoch)
	...
	def fit(self, dataset: BaseModel.Dataset, epoch=-1, writer=None) -> float:
		model = dataset.model
	...
	def evaluate(self, dataset: BaseModel.Dataset, topks: list, metrics: list, writer=None, epoch=0) -> Dict[str, float]:
		"""
		Evaluate the results for an input dataset.
		:return: result dict (key: metric@k)
		"""
		predictions = self.predict(dataset)
		evaluate_dict = self.evaluate_method(predictions, topks, metrics)
		if writer is not None:
			writer.add_scalar('%s_NDCG%d'%(dataset.phase,topks[0]),evaluate_dict['NDCG@%d'%(topks[0])],global_step=epoch)
	...
	def predict(self, dataset: BaseModel.Dataset) -> np.ndarray:
		...
		for batch in tqdm(dl, leave=False, ncols=100, mininterval=1, desc='Predict'):
			...
			# if len(predictions) in [20*256,40*256,60*256]:
			# 	print("Last inference")
